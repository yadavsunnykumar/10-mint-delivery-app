...
import {
  getGridCategoryBySlug,
  type GridCategoryConfig,
} from "@/lib/categories";
import { fetchProducts } from "@/lib/api";
import type { LayoutContext } from "@/components/AppLayout";

// ——— Inner content (hooks always called, no conditional hook issue) ———————————
const ShopContent = ({ config }: { config: GridCategoryConfig }) => {
  const { searchQuery } = useOutletContext<LayoutContext>();

  const { data: products, isLoading } = useQuery({
    queryKey: ["shop-products", config.backendCategory, searchQuery],
    queryFn: () =>
      fetchProducts({
  ...
  return (
    <>
      ...
      {/* Category grid — shown on every shop page */}
      <CategoryGrid />
...
// ——— Route wrapper ——————————————————————————————————————————————————————————
const ShopPage = () => {
  const { gridSlug } = useParams<{ gridSlug: string }>();
  const navigate = useNavigate();
  const config = getGridCategoryBySlug(gridSlug ?? "");
  ...
  return <ShopContent config={config} />;
};
